...
\begin{tikzpicture}[
  ...
  \node[gbox] (fus) at (4.5,-1.8){União das máscaras\\+ abertura direcional\\+ morfologia};
...
\[
  0{,}65\!\cdot\!\text{linear} + 0{,}20\!\cdot\!\text{homog} + 0{,}15\!\cdot\!(1\!-\!s)
\]
...
\begin{itemize}
  ...
  \item NDVI-like não filtra solo de cor amarela/bege
...
\begin{columns}
\column{0.48\linewidth}
\textbf{Malha radial:} estrutura circular captada; avenidas principais detectadas; telhados geram ruído
\column{0.48\linewidth}
\textbf{Malha em grade:} ruas da grade visíveis na sobreposição; blobs de telhado ainda presentes; score urbano melhora em relação à V2
\end{columns}
}
\end{frame}
...
Falsos positivos       & Muito alto      & Alto $\rightarrow$ Médio (ajustes)    & Médio (urbano melhor; rural difícil) \\
\bottomrule
\end{tabularx}
}
...
\begin{itemize}
  \item Cada versão parte do diagnóstico da anterior
...
\begin{itemize}
  \item Pipeline clássico (sem CNN) funcional para ambos contextos
  \item Máscara melhorou consistentemente a cada iteração
  \item Grafo implementado; qualidade limitada pela máscara
  \item Código modular e reutilizável para próximas iterações
\end{itemize}
\end{columns}
